...
void regCreateAndSetValue() {
	...
	wcsncpy_s(Data, TEXT("https://github.com/Hornos3"), 254);
	...
	// 设置键值
	lRet = RegSetValueEx(hKey, L"SuDongXun", 0, REG_SZ, (CONST BYTE*)Data, sizeof(TCHAR) * iLen);
	if (lRet == ERROR_SUCCESS)
	{
...
void regOpenAndDelValue() {
	...
	lRet = RegDeleteValue(hKey, L"SuDongXun");
	if (lRet == ERROR_SUCCESS) {
